...
const COMMON_FONT_FAMILIES = [
	{ label: __( '— Select —', 'multisite-override-style' ), value: '' },
...
export default function Typography( { fontFamilies, fontSizes, onChange } ) {
	...
	return (
		<>
			<Panel>
				<PanelBody title={ __( 'Font Families', 'multisite-override-style' ) } initialOpen={ true }>
					{ fontFamilies.map( ( family, i ) => (
						<div key={ i } className="mos-font-family-row">
							<TextControl
								label={ __( 'Name', 'multisite-override-style' ) }
								value={ family.name }
								onChange={ ( v ) => updateFamily( i, 'name', v ) }
							/>
							<TextControl
								label={ __( 'Slug', 'multisite-override-style' ) }
								value={ family.slug }
								onChange={ ( v ) => updateFamily( i, 'slug', v ) }
							/>
							<TextControl
								label={ __( 'Font Family CSS', 'multisite-override-style' ) }
								value={ family.fontFamily }
								onChange={ ( v ) => updateFamily( i, 'fontFamily', v ) }
								placeholder="e.g. Georgia, serif"
							/>
							<Button isDestructive variant="tertiary" onClick={ () => removeFamily( i ) }>
			...
			<Panel>
				<PanelBody title={ __( 'Font Sizes', 'multisite-override-style' ) } initialOpen={ false }>
					{ fontSizes.map( ( size, i ) => (
						<div key={ i } className="mos-font-size-row">
							...
							/>
							<Button isDestructive variant="tertiary" onClick={ () => removeSize( i ) }>
								{ __( 'Remove', 'multisite-override-style' ) }
							</Button>
						</div>
					) ) }
